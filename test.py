...
def test(data,
        ...
        with torch.no_grad():
            ...
            if training:  # if model has loss hyperparameters
                loss += compute_loss([x.float() for x in train_out],
                                     targets, model)[1][:3]  # GIoU, obj, cls

            # Run NMS
            t = torch_utils.time_synchronized()
            output = non_max_suppression(inf_out, conf_thres=conf_thres,
                                         iou_thres=iou_thres, merge=merge)
    ...
    if save_json and map50 and len(jdict):
        ...
        try:
            ...
            cocoGt = COCO(glob.glob('/home/amit/pic2card_yolo_train/yolov5/template_gt.json')[0])
            print(cocoGt)
